#include<stdio.h>
#include <stdlib.h>

//creating node datatype

struct node
{
    int data;
    struct node* next;
};
typedef struct node node;

//insert node at begining

node* front(node *head,node *new_node,node *tail)
{
    new_node->next=head;
    head=new_node;
    tail->next=new_node;    
    return head;
}

//insert node at end

node* end(node *head,node* new_node,node *tail)
{
    tail->next=new_node;
    new_node->next=head;
}

//insert node at position

node *between(node *head,int pos,node* new_node)
{
    node *temp;
    temp=head;
    for(int i=1;i<pos-1;i++)
    {
        temp=temp->next;
    }
    new_node->next=temp->next;
    temp->next=new_node;
}

//delete node at begining

node *dfront(node *head,node *tail)
{
    node *temp=head;
    head=head->next;
    tail->next=head;
    free(temp);
    return head;
}

//delete node at end

node *dend(node *head,node *tail)
{
    node *temp=head; 
    while(temp->next!=tail)
    {
        temp=temp->next;
    }
    free(tail);
    tail=temp;
    tail->next=head;
}

//delete node at position

node *dpos(node *head,int pos)
{
    node *temp;
    node *next_node;
    temp=head;
    for(int i=1;i<pos-1;i++)
    {
        temp=temp->next;
    }
    next_node=temp->next;
    temp->next=next_node->next;
    free(next_node);
    
}


//display the list

node* display(node *head)
{
    node *temp;
    temp=head;
    do
    {
        printf("%d  ",temp->data);
        temp=temp->next;
    }while(temp!=head);
}





int main()
{
    int choice,insertion_choice,value,k,n,pos;
    node *head=NULL,*new_node,*temp,*tail;
    printf("The list is empty\n");
    printf("-------------------------------\n");
    printf("Enter the number of elements to be insert:  ");
    scanf("%d",&n);
	
    //creating single linked list

    for(int i=0;i<n;i++)
    {
        new_node=(node*)malloc(sizeof(node));
        printf("Enter the element to be insert:  ");
        scanf("%d",&new_node->data);
        if(head==NULL)
        {
            head=new_node;
            tail=new_node;
        }
        else
        {
            tail->next=new_node;
            tail=new_node;
        }
        tail->next=head;
   }
    printf("-------------------------------\n");
    display(head);
    printf("\n-------------------------------\n");
    printf("\n");
    do{
        printf("Select the operation:\n1.Insertion\n2.Deletion\n");
        printf("-------------------------------\n");
        scanf("%d",&choice);

	//insertion
	
        if(choice==1)
        {
            printf("Select where to insert:\n1.insert at begining\n2.insert at end\n3.insert at position\n");
            printf("-------------------------------\n");
            scanf("%d",&insertion_choice);
            new_node=(node*)malloc(sizeof(node));
            printf("Enter the data to be insert: ");
            scanf("%d",&new_node->data);
            if(insertion_choice==1)
            {
            head=front(head,new_node,tail);
            }
            else if(insertion_choice==2)
            {
            end(head,new_node,tail);
            }
            else if(insertion_choice==3)
            {
            printf("Enter the position you want to insert: ");
            scanf("%d",&pos);
            between(head,pos,new_node);
            }
            printf("-------------------------------\n");
            display(head);
            printf("\n-------------------------------\n");
        }
	
	//delection
	
        else if(choice==2)
        {
            printf("Select where to delete:\n1.delete at begining\n2.delete at end\n3.delete at position\n");
            printf("-------------------------------\n");
            int deletion_choice;
            scanf("%d",&deletion_choice);
            if(deletion_choice==1)
            {
                head=dfront(head,tail);
            }
            else if(deletion_choice==2)
            {
		dend(head,tail);
            }
            else if(deletion_choice==3)
            {
                printf("Enter the position you want to delete: ");
                scanf("%d",&pos);
                dpos(head,pos);
            }
            printf("-------------------------------\n");
            display(head);
            printf("\n-------------------------------\n");
            
        }
        printf("\nIf you want to continue insert again enter 1 Else 0: ");
        scanf("%d",&k);
    }while(k);
}
